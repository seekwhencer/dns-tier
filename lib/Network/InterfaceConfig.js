    ...
    save() {
        ...
        Object.keys(this.options).forEach(key => {
            ...
            if (iface.dhcp === true) {
                ...
                iface.address ? fileData += `address ${iface.address}\n` : null;
                iface.netmask ? fileData += `netmask ${iface.netmask}\n` : null;
                iface.gateway ? fileData += `gateway ${iface.gateway}\n` : null;
                iface['dns-nameservers'] ? fileData += `dns-nameservers ${iface['dns-nameservers']}\n` : null;
            }
            fileData += `\n`;
        });
