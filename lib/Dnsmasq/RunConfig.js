...
export default class Dnsmasqfile extends Module {
    ...
    save() {
        ...
        dynamic["dhcp-leasefile"] === true ? fileData += `dhcp-leasefile=${this.dhcpFile}\n` : null;
        dynamic["conf-dir"] === true ? fileData += `conf-dir=${this.path}/mapping/,*.conf\n` : null;
        dynamic["hosts-dir"] === true ? fileData += `hosts-dir=${this.path}/mapping/,*.conf\n` : null;
    ...
    set path(val){
        this._path = val;
        this.dhcpFile = `${this.path}/dhcp.leases`;
    }
}